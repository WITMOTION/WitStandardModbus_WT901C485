...
BuffIndex3=0;
DisplayIndex=1;
aa=zeros(1,devNo*3); %每个设备的加速度有3个轴，1，2，3为1号设备，4，5，6为2号设备，......34,35,36为1号设备   There are 3 axes acceleration of each device,  device 1 is 1, 2, 3, device 2 is 4, 5, 6 ... device 1 is 34, 35, 36 
ww=zeros(1,devNo*3);
AA=zeros(1,devNo*3);
tt=zeros(1,1);
a=zeros(1,devNo*3);  %每个设备的加速度有3个轴，1，2，3为1号设备，4，5，6为2号设备，......34,35,36为1号设备   
w=zeros(1,devNo*3);
A=zeros(1,devNo*3);
Temp=zeros(1,24);

UnLockCmd=[80 06 0 105 181 136 34 161]';  %0xFF 0xAA 0x69 0x88 0xB5
CaliCmd=[80 06 0 1 0 7 148 73]';  %0xFF 0xAA 0x01 0x07 0x00
OutCaliCmd=[80 06 0 1 0 0 213 139]';  %0xFF 0xAA 0x01 0x00 0x00
SaveCmd=[80 06 0 0 0 0 132 75]';  %0xFF 0xAA 0x00 0x00 0x00
...
while(1)
    if CalFlag==0
        ...
        if strcmp(CalFlag,StartCali)==1
            ...
            pause(1);
            fwrite(s,CaliCmd(1:8),'uint8');                 %解锁完成后，等待1秒，发送进入磁场校准   After unlocking, wait for 1 second and send command to enter the magnetic field calibration
            disp('校准中，请分别绕3个轴慢速各旋转一圈，60秒后自动退出校准')
            pause(60);
            fwrite(s,UnLockCmd(1:8),'uint8');               %解锁   Unlock
            pause(1);
            fwrite(s,OutCaliCmd(1:8),'uint8');              %等待60秒，校准完成后，退出校准   Wait 60 seconds, after the calibration is complete, exit the calibration
            pause(1);
            fwrite(s,SaveCmd(1:8),'uint8');                 %等待1秒，保存校准参数   Wait 1 second to save the calibration parameters
    ...
    for NumCnt=1:devNo
        ...
        w(BuffIndex1)=single(int16(Temp(7)*256+Temp(8)))/32768*2000;%取角速度X   Get angular velocity Y
        w(BuffIndex2)=single(int16(Temp(9)*256+Temp(10)))/32768*2000;%取角速度Y   Get angular velocity Y
        w(BuffIndex3)=single(int16(Temp(11)*256+Temp(12)))/32768*2000;%取角速度Z   Get angular velocity Y
        A(BuffIndex1)=single(int16(Temp(19)*256+Temp(20)))/32768*180;%取角度X   Get angle Y
        A(BuffIndex2)=single(int16(Temp(21)*256+Temp(22)))/32768*180;%取角度Y   Get angle Y
        A(BuffIndex3)=single(int16(Temp(23)*256+Temp(24)))/32768*180;%取角度Z   Get angle Y
    end                                                              
